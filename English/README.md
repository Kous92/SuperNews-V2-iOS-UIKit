...
- **Factory:** Pattern which delegates the object instantiation to a distinct method, without exposing his creation logic. It is used when there is several concrete classes possibilities from an abstraction, or several object instantiation possibilities depending of one or multiple input parameters. Here, I use it for example when the `TableView` can use multiple versions of custom `TableViewCell`.
