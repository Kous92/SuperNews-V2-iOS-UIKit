...
- **Factory**: Le pattern qui permet de déléguer l'instanciation d'un objet à une méthode distincte, sans exposer la logique de création. Il est utilisé principalement lorsqu'il y a soit plusieurs possibilités de classes concrètes depuis une abstraction, soit plusieurs possibilités d'initialisation d'un objet en fonction de un ou plusieurs paramètres fournis en entrée. Ici, je l'utilise par exemple quand le `TableVIew` peut utiliser plusieurs versions de `TableViewCell` personnalisés.
